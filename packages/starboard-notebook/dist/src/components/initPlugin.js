...
export const setupPyqe = async (runtime, serverUrl, token) => {
    if (!serverUrl || !token) {
        return;
    }
...
os.environ['PYQE_TLS_CLIENT_CA_CERT_PATH'] = ''`;
    const pythonRuntime = runtime.plugins.get("starboard-python");
    try {
        const val = await pythonRuntime.exports.runStarboardPython(runtime, PYQE_INSTALL_CODE);
        if (val) {
...
export const initPlugin = async (runtime, serverUrl, token) => {
    try {
